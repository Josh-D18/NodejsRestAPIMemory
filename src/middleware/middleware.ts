...
const validateOverDueDate = (date: string, res: Response, user: UserRecord): boolean => {
    //
    if (date.length === 0) {
        return false;
    }else{
...
const handleUserFunction = async function (req: Request, res: Response, next: NextFunction): Promise<void> {
...
const handleOverDueFunction = async function (req: Request, res: Response, next: NextFunction): Promise<void>  {
    const filePath = path.join(__dirname, "../storage", "data.json");
